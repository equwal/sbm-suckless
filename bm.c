...
#define KEYS "ctrl-y,ctrl-a,ctrl-d,ctrl-e,ctrl-t,ctrl-s"
...
/* picktag: set the tag filter from a menu of every tag in the file. */
static void picktag(void) {
    struct buf in = {0}, tags = {0};
    char **names, *tag;
    size_t n, i;
    ...
    if (*tag) {
        free(tagfilter);
        tagfilter = xstrdup(strcmp(tag, "(all)") ? tag : "");
    }
    free(tag);
    freenames(names, n);
    free(in.s);
...
static int browsefzf(void) {
    ...
    while (rc < 0) {
        ...
        arg(&a, "--expect=" KEYS);
        bufprintf(&opt,
            "--header=ENTER open  ^Y copy  ^A add  ^D delete  ^E edit  ^T tag [%s]  ^S sort [%s]",
            *tagfilter ? tagfilter : "all", orders[order]);
        arg(&a, opt.s);
